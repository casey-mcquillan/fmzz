    ...
    def calibrate(self):
        ...
        def equations(vars):
            ...
            eq2 = w0_c-lambda_c*L0_c**(rho-1) \
                *(lambda_n*L0_n**(rho)+lambda_c*L0_c**(rho))**(1/rho-1)
            eq3 = L0_n - share_pop_n*((w0_n-kappa_lb)/(kappa_ub-kappa_lb))
            eq4 = L0_c - share_pop_c*((w0_c-kappa_lb)/(kappa_ub-kappa_lb))
            return [eq1, eq2, eq3, eq4]
        #Let initial guess be based on Head Tax Eq   
        w0_n_guess = w1_n + tau
        w0_c_guess = w1_c + tau
        L0_n_guess = share_pop_n*((w0_n_guess+alpha_n*tau-kappa_lb)/(kappa_ub-kappa_lb))
        L0_c_guess = share_pop_c*((w0_c_guess+alpha_c*tau-kappa_lb)/(kappa_ub-kappa_lb))
        ...
        general_params = ['tau', 'pop_count', 'kappa_dist', 'kappa_lb', 'kappa_ub', 't']
        for var in general_params:
            exec("self."+var+"="+var)
            
        general_params_w_type = ['alpha','lambda', 'share_pop', ]
...
model.generate_table(file_name='SummaryTable'+str(year)+"_RC3", year=year, 
...
model.generate_table(file_name='EqComparison'+str(year)+"_RC3", year=year, 
